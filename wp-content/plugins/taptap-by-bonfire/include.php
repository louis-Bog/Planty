...
<!-- BEGIN WOO CART -->
<?php if(get_theme_mod('taptap_show_woo_button', '') !== '') { ?>
    <?php include_once( ABSPATH . 'wp-admin/includes/plugin.php' ); if ( is_plugin_active( 'woocommerce/woocommerce.php' ) ) { ?>
        <div class="taptap-woo-button-wrapper<?php if ( is_admin_bar_showing() ) { ?> wp-toolbar-active<?php } ?><?php if(get_theme_mod('taptap_absolute_position', '') !== '') { ?> taptap-absolute<?php } ?>">
            <a class="taptap-cart-count" href="<?php echo wc_get_cart_url(); ?>" title="<?php esc_html_e('View your shopping cart','bonfire'); ?>">
...
<div class="taptap-search-wrapper<?php if(get_theme_mod('taptap_absolute_position', '') !== '') { ?> taptap-absolute<?php } ?><?php if ( is_admin_bar_showing() ) { ?> wp-toolbar-active<?php } ?>">
        ...
        <form method="get" id="searchform" action="<?php echo esc_url( home_url('') ); ?>/">
            <input type="text" name="s" class="taptap-search-field" placeholder="<?php if(get_theme_mod('taptap_search_placeholder', '') !== '') { ?><?php echo get_theme_mod('taptap_search_placeholder'); ?><?php } else { ?><?php esc_html_e( 'enter search term' , 'bonfire' ) ?><?php } ?>">
        </form>
</div>
<!-- END SEARCH FORM -->
            ...
            <div class="taptap-logo-image">
                <a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" rel="home">
                    <div class="taptap-logo-primary">
                        <img src="<?php echo get_theme_mod('taptap_logo_image'); ?>" data-rjs="3" alt="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>">
                    </div>
                    <?php if( get_theme_mod('taptap_secondary_logo_image', '') !== '') { ?>
                        <div class="taptap-logo-secondary">
                            <img src="<?php echo get_theme_mod('taptap_secondary_logo_image'); ?>" alt="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>">
                        </div>
                    <?php } ?>
                </a>
            </div>
...
<?php if(get_theme_mod('taptap_show_header', '') !== '') { ?>
    <div class="taptap-header-bg-image<?php if ( is_admin_bar_showing() ) { ?> wp-toolbar-active<?php } ?><?php if(get_theme_mod('taptap_absolute_position', '') !== '') { ?> taptap-absolute<?php } ?>">
    </div>
<?php } ?>
...
<div class="taptap-main-wrapper<?php if(get_theme_mod('taptap_open_on_front_page')!='' && is_front_page() ) { ?> taptap-main-wrapper-active<?php } ?>">
    <div class="taptap-main-inner">
        ...
        <?php if( get_theme_mod('taptap_show_alt_close', '') !== '') { ?>
            ...
            <div class="taptap-main-inner-inner<?php if ( is_admin_bar_showing() ) { ?> taptap-main-inner-inner-toolbar<?php } ?>">
                <div class="taptap-contents-wrapper">
                    ...
                    <div class="taptap-by-bonfire-image-wrapper">
                        <?php $walker = new TapTap_Image_Menu_Wrappers; ?>
                        <?php wp_nav_menu( array( 'container_class' => 'taptap-by-bonfire-image', 'theme_location' => 'taptap-by-bonfire-image', 'walker' => $walker, 'fallback_cb' => '' ) ); ?>
                    </div>
                    <!-- END IMAGE-BASED MENU -->

                    <!-- BEGIN HORIZONTAL MENU -->
                    <div id="taptap-move-horiz" class="taptap-by-bonfire-horizontal">
                        <?php wp_nav_menu( array( 'theme_location' => 'taptap-by-bonfire-horizontal', 'fallback_cb' => '' ) ); ?>
                    </div>
                    <!-- END HORIZONTAL MENU -->
...
<?php if(get_theme_mod('taptap_open_menu_on_hover', '') !== '' && !wp_is_mobile() ) { ?>
...
'use strict';
    e.preventDefault();
        if(jQuery('.taptap-background-color').hasClass('taptap-background-color-active')) {
            ...
            jQuery('body').addClass('taptap-body-overflow');
            <?php } ?>
...
'use strict';
	e.preventDefault();
		if(jQuery('.taptap-background-color').hasClass('taptap-background-color-active')) {
            ...
            setTimeout(function() {
                jQuery('body').removeClass('taptap-body-overflow');
            }, <?php if(get_theme_mod('taptap_menu_appearance_speed', '') !== '') { ?><?php echo get_theme_mod('taptap_menu_appearance_speed'); ?><?php } else { ?>0.5<?php } ?> * 1000);
            <?php } ?>
...
.taptap-content-animation-active {
    -webkit-transform:scale(<?php if(get_theme_mod('taptap_content_animation_scale', '') !== '') { ?><?php echo get_theme_mod('taptap_content_animation_scale'); ?><?php } else { ?>0.9<?php } ?>) <?php if(get_theme_mod('taptap_content_animation_movex', '') !== '') { ?>translateX(<?php echo get_theme_mod('taptap_content_animation_movex'); ?>px)<?php } ?> <?php if(get_theme_mod('taptap_content_animation_movey', '') !== '') { ?>translateY(<?php echo get_theme_mod('taptap_content_animation_movey'); ?>px)<?php } ?>;
    transform:scale(<?php if(get_theme_mod('taptap_content_animation_scale', '') !== '') { ?><?php echo get_theme_mod('taptap_content_animation_scale'); ?><?php } else { ?>0.9<?php } ?>) <?php if(get_theme_mod('taptap_content_animation_movex', '') !== '') { ?>translateX(<?php echo get_theme_mod('taptap_content_animation_movex'); ?>px)<?php } ?> <?php if(get_theme_mod('taptap_content_animation_movey', '') !== '') { ?>translateY(<?php echo get_theme_mod('taptap_content_animation_movey'); ?>px)<?php } ?>;
